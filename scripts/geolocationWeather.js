//TODO Add click event for search button and handle all that jazz
...
function displayData(data) {
    ...
    if (presentTime.substring(0,2) == 0) {
        console.log(presentTime.substring(0,2));
...
function success(pos) {
    //Getting coordinates via builtin geolocation api
    let coords = pos.coords;
    let longitude = coords.longitude;
    let latitude = coords.latitude;

    //Location in decimal degree format
    let q = `${latitude},${longitude}`;

    //Calling weatherapi
    fetch(`https://api.weatherapi.com/v1/forecast.json?key=b144387a316747d9b07163114220307&q=${q}&days=7&aqi=no&alerts=no`)
